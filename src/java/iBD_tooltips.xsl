 ...
 * @author DM Smith [dmsmith555 at yahoo dot com]
 ...
 <xsl:stylesheet
  ...
  <xsl:param name="Morph" select="'true'"/>

  <!-- Whether to start each verse on an new line or not -->
  <xsl:param name="VLine" select="'true'"/>
  ...
  <xsl:template name="versenum">
    ...
    <xsl:if test="$NoVNum = 'false'">
      ...
      <xsl:choose>
		...
		<a href="bible://{@osisID}" name="{@osisID}" onclick='return handlePassage(&quot;{@osisID}&quot;);'><sup class="verse"><xsl:value-of select="$versenum"/></sup>  </a>
        </xsl:when>
  ...
  <xsl:template name="lemma">
    ...
    <xsl:choose>
      <xsl:when test="$separator = ''">
        ...
        <sub class="strongs"><button data-html='true' rel='tooltip' data-original-title='' onclick='doStrongs(this,&quot;{$protocol}&quot;,&quot;{$orig-lemma}&quot;);'><xsl:value-of select="format-number(substring($orig-lemma,2),'#')"/></button></sub>
      </xsl:when>
      <xsl:otherwise>
        ...
        <sub class="strongs"><button data-html='true' rel='tooltip' data-original-title='' onclick='doStrongs(this,&quot;{$protocol}&quot;,&quot;{substring-before($orig-lemma, $separator)}&quot;);'><xsl:value-of select="format-number(substring(substring-before($orig-lemma, $separator),2),'#')"/></button>, </sub>
        <xsl:call-template name="lemma">
  ...
  <xsl:template name="morph">
    ...
    <xsl:choose>
      <xsl:when test="$separator = ''">
        ...
        <sub class="morph"><button data-html='true' rel='tooltip' data-original-title='' onclick='doStrongs(this,&quot;{$protocol}&quot;,&quot;{$orig-morph}&quot;);'><xsl:value-of select="$orig-morph"/></button></sub>
      </xsl:when>
      <xsl:otherwise>
        ...
        <sub class="morph"><button data-html='true'  rel='tooltip' data-original-title='' onclick='doStrongs(this,&quot;{$protocol}&quot;,&quot;{substring-before($orig-morph, $separator)}&quot;);'><xsl:value-of select="substring-before($orig-morph, $separator)"/></button>, </sub>
        <xsl:call-template name="morph">
  ...
  <xsl:template match="divineName">
    <span class="divineName"><xsl:apply-templates mode="small-caps"/></span>
  </xsl:template>
  ...
  <xsl:template match="inscription">
    <span class="inscription"><xsl:apply-templates mode="small-caps"/></span>
  </xsl:template>
